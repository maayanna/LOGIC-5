...
def prove_and_commutativity() -> Proof:
    ...
    line_ea1 = Proof.Line(Formula.parse("q"), AE1_RULE, [0])
    line_ea2 = Proof.Line(Formula.parse("p"), AE2_RULE, [0])
    line_ea = Proof.Line(Formula.parse("(q&p)"), A_RULE, [1,2])

    statement = InferenceRule([Formula.parse("(p&q)")], Formula.parse("(q&p)"))

    rules ={ A_RULE, AE1_RULE, AE2_RULE }

    return Proof(statement,rules, [Proof.Line(Formula.parse("(p&q)")), line_ea1, line_ea2, line_ea])
...
def prove_I0() -> Proof:
    ...
    """
    # Task 4.8
    line_D = Proof.Line(Formula.parse("((p->((p->p)->p))->((q->(q->p))->(p->p)))"), D, [])
    line_I1 = Proof.Line(Formula.parse("(q->(p->q))"), I1, [])
    line_MP = Proof.Line(Formula.parse("((p->(p->p))->(p->p))"), MP, [1,0])
    line_from_I1 = Proof.Line(Formula.parse("(p->(p->p))"), I1, [])
    line_from_MP = Proof.Line(Formula.parse("(p->p)"), MP, [3,2])

    statement = InferenceRule([], Formula.parse("(p->p)"))

    rules ={ I1, MP, D }

    return Proof(statement,rules, [line_D, line_I1, line_MP, line_from_I1, line_from_MP])
...
def prove_hypothetical_syllogism() -> Proof:
    ...
    first_line = Proof.Line(Formula.parse("(p->q)"))
    scd_line = Proof.Line(Formula.parse("(q->r)"))
    third = Proof.Line(Formula.parse("((p->(q->r))->((p->q)->(p->r)))"), D, [])
    fourth = Proof.Line(Formula.parse("((q->r)->(p->(q->r)))"), I1, [])
    fifth = Proof.Line(Formula.parse("(p->(q->r))"), MP, [1, 3])
    six = Proof.Line(Formula.parse("((p->q)->(p->r))"), MP, [4, 2])
    seven_line = Proof.Line(Formula.parse("(p->r)"), MP, [0, 5])

    my_lines = [first_line, scd_line, third, fourth, fifth, six, seven_line]
    my_rules = {MP, I0, I1, D}

    return Proof(HS, my_rules, my_lines)
